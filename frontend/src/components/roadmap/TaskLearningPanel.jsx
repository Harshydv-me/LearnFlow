  ...
  return (
    <div className="fixed inset-0 z-50">
      <div
        ...
        onClick={onClose}
      />
      <div className="fixed right-0 top-0 h-full w-full animate-slide-in border-l border-[#1f1f1f] bg-[#0f0f0f] shadow-2xl shadow-black/50 sm:w-[600px] lg:w-[680px]">
        <div className="flex h-full flex-col">
          <div className="flex-shrink-0 border-b border-[#1f1f1f] bg-[#0f0f0f] px-6 py-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center text-xs text-[#666]">
                <span>{skillName}</span>
                <ChevronRight size={12} className="mx-1 text-[#444]" />
                <span>{topic?.title}</span>
              </div>
              <button
                onClick={onClose}
                className="rounded-lg p-1.5 text-[#666] transition-all duration-200 hover:bg-[#1f1f1f] hover:text-white"
              >
                <X size={16} />
              </button>
            </div>

            <div className="mt-2 text-xl font-bold text-white">{task.title}</div>
            <div className="mt-1 text-sm text-[#666]">{task.description}</div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 [scrollbar-width:thin] [scrollbar-color:#333_transparent] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#333]">
            {loadingContent ? (
              <div>
                <div className="mb-6 flex items-center gap-2">
                  <Sparkles size={16} className="text-[#6366f1]" />
                  <span className="text-sm text-[#666]">Generating your lesson...</span>
                ...
                <div className="mt-4 h-4 w-full animate-pulse rounded bg-[#1f1f1f]" />
                <div className="mt-2 h-4 w-5/6 animate-pulse rounded bg-[#1f1f1f]" />
                <div className="mt-2 h-4 w-4/6 animate-pulse rounded bg-[#1f1f1f]" />
                <div className="mt-6 h-32 w-full animate-pulse rounded-xl bg-[#1f1f1f]" />
                <div className="mt-6 h-4 w-full animate-pulse rounded bg-[#1f1f1f]" />
                <div className="mt-2 h-4 w-3/4 animate-pulse rounded bg-[#1f1f1f]" />
              </div>
            ) : contentError ? (
              <div className="rounded-xl border border-red-500/20 bg-[#1a1a1a] p-6 text-center">
                <AlertCircle size={24} className="mx-auto text-red-400" />
                <div className="mt-2 text-sm text-white">Failed to generate content</div>
                <div className="mt-1 text-xs text-[#666]">
                  Check your API key or try again
                </div>
                <button
                  onClick={() => setRetryCount((count) => count + 1)}
                  className="mt-4 rounded-lg bg-[#1f1f1f] px-4 py-2 text-sm text-white transition-all duration-200 hover:bg-[#2a2a2a]"
                >
                  Try Again
                </button>
              </div>
              ...
              <div>
                ...
                <div className="mt-8">
                  <div className="mb-4 flex items-center gap-2">
                    ...
                    <div className="text-sm font-semibold text-white">Code Examples</div>
                  </div>

                  {content?.codeExamples?.map((example, index) => (
                    <div
                      key={`${example.title}-${index}`}
                      className="mb-4 overflow-hidden rounded-xl border border-[#1f1f1f] bg-[#0a0a0a]"
                    >
                      <div className="flex items-center justify-between bg-[#111] px-4 py-2.5">
                        ...
                        <span className="text-xs text-[#666]">{example.title}</span>
                      </div>
                      <SyntaxHighlighter
                        ...
                        showLineNumbers
                        lineNumberStyle={{ color: "#333", fontSize: "11px" }}
                      >
                        {example.code}
                      </SyntaxHighlighter>
                      <div className="border-t border-[#1f1f1f] px-4 py-3 text-xs italic text-[#666]">
                        {example.explanation}
                      </div>
                    </div>
                  ))}
                ...
                <div className="mt-8">
                  <div className="mb-3 flex items-center gap-2">
                    ...
                    <div className="text-sm font-semibold text-white">Key Takeaways</div>
                  </div>
          ...
          <div className="flex-shrink-0 border-t border-[#1f1f1f] bg-[#0f0f0f] px-6 py-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                {task.completed ? (
                  ...
                  <div className="text-xs text-[#444]">Complete previous tasks first</div>
                )}
              </div>
                  ...
                  <button
                    ...
                    className={`rounded-lg px-4 py-2.5 text-sm transition-all duration-200 ${
                      nextTask.unlocked || task.completed
                        ...
                        : "cursor-not-allowed border border-[#1f1f1f] text-[#444] opacity-50"
                    }`}
                  >
                    Next Task →
                  </button>
