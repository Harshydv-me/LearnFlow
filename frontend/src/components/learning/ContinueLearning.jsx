...
const ContinueLearning = ({ topic, firstUnlockedTask, loading }) => {
  ...
  if (loading) {
    return (
      <div className="rounded-xl border border-[#1f1f1f] bg-[#111] p-6">
        ...
        <div className="mt-4 h-6 w-64 animate-pulse rounded bg-[#1f1f1f]" />
        <div className="mt-2 h-4 w-80 animate-pulse rounded bg-[#1f1f1f]" />
        <div className="mt-4 h-2 w-full animate-pulse rounded bg-[#1f1f1f]" />
      </div>
    );
  }
  ...
  if (!topic) {
    return (
      <div className="rounded-xl border border-[#1f1f1f] bg-[#111] p-6 text-center">
        ...
        <div className="mt-3 text-lg font-semibold text-white">All caught up!</div>
  ...
  return (
    <div className="rounded-xl border border-[#1f1f1f] bg-[#111] p-6">
      ...
      <div className="mt-3 text-2xl font-bold text-white">{topic.title}</div>
      <div className="mt-1 text-sm text-[#666]">{topic.description}</div>

      <div className="mt-4 h-1.5 w-full rounded-full bg-[#1f1f1f]">
        <div
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <button
          onClick={() => navigate("/roadmap")}
          className="rounded-lg border border-[#1f1f1f] px-5 py-2.5 text-sm text-[#666] transition-all duration-200 hover:border-[#333] hover:text-white"
        >
          View Roadmap
        </button>
      </div>
