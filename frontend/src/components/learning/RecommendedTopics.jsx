    ...
    return (
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        {Array.from({ length: 2 }).map((_, idx) => (
          <div key={idx} className="h-24 w-full animate-pulse rounded-xl bg-skeleton" />
        ))}
      </div>
    );
  }
  ...
  if (!topics || topics.length === 0) {
    return (
      <div className="rounded-xl border border-subtle bg-card p-6 text-center">
        ...
        <div className="mt-3 text-sm text-secondary">No recommendations yet</div>
      ...
      {topics.map((topic) => (
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="rounded-full bg-card-hover px-2 py-0.5 text-xs text-secondary">
              {topic.skillName}
            </span>
          </div>
          <div className="mt-3 text-sm font-semibold text-primary">{topic.title}</div>
          <div className="mt-1 text-xs text-secondary">{topic.description}</div>
