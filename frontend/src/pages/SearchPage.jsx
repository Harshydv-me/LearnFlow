  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white">
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        <div
          className="mb-6 flex items-center gap-2 text-sm text-[#666] transition-colors duration-200 hover:text-white"
          onClick={() => navigate(-1)}
          role="button"
          tabIndex={0}
          onKeyDown={(event) => {
        ...
        {loading && (
          <div className="mt-8 space-y-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div
                key={`skeleton-${index}`}
                className="h-20 rounded-xl border border-[#1f1f1f] bg-[#111] animate-pulse"
              />
            ))}
          </div>
        )}
        ...
        {!loading && totalResults === 0 && (
          <div className="mt-16 text-center">
            <Search size={48} className="mx-auto text-[#333]" />
        ...
        {!loading && results.skills.length > 0 && (
          <section className="mt-8">
            <div className="mb-3 flex items-center text-sm font-semibold text-white">
              Skills
              <span className="ml-2 rounded-full bg-[#1f1f1f] px-2 py-0.5 text-xs text-[#666]">
            ...
            {results.skills.map((skill) => (
              <div
                ...
                className="mb-3 flex cursor-pointer items-center gap-4 rounded-xl border border-[#1f1f1f] bg-[#111] p-5 transition-all duration-200 hover:border-[#333]"
              >
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-0.5 line-clamp-1 text-sm text-[#666]">
                    {skill.description}
                  </div>
                </div>
          ...
          <section className="mt-8">
            ...
            {results.topics.map((topic) => (
              <div
                ...
                className="mb-3 cursor-pointer rounded-xl border border-[#1f1f1f] bg-[#111] p-5 transition-all duration-200 hover:border-[#333]"
              >
                ...
                <div className="ml-14 mt-2 text-xs text-[#666]">
                  <span className="rounded bg-[#1a1a1a] px-2 py-0.5">
                    {topic.skillName}
                  </span>
                </div>
                <div className="ml-14 mt-2 line-clamp-2 text-sm text-[#666]">
                  {topic.description}
                </div>
